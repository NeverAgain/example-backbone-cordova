define (require) ->
  ...
  findByName = (searchKey) ->
    deferred = $.Deferred()
    results = employees.filter((element) ->
      fullName = element.firstName + " " + element.lastName
      fullName.toLowerCase().indexOf(searchKey.toLowerCase()) > -1
    )
  ...
  findByManager = (managerId) ->
    deferred = $.Deferred()
    results = employees.filter((element) ->
      managerId is element.managerId
    )
    deferred.resolve results
    deferred.promise()
  ...
  Employee = Backbone.Model.extend(
    ...
    sync: (method, model, options) ->
      if method is "read"
        findById(parseInt(@id)).done (data) ->
  ...
  EmployeeCollection = Backbone.Collection.extend(
    ...
    sync: (method, model, options) ->
      if method is "read"
        findByName(options.data.name).done (data) ->
          options.success data
          return
  ...
  ReportsCollection = Backbone.Collection.extend(
    ...
    sync: (method, model, options) ->
      if method is "read"
        findByManager(@parent.id).done (data) ->
          options.success data
          return
